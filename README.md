...
- **幻灯片**(顶栏「▶ 幻灯片」,没选就播当前筛选):
  - **真全屏**(Fullscreen API,占满整屏黑底);淡出→全黑→淡入,不重叠(各 0.3s,可调);停留**默认 8s**(2/8/30s 或自定义任意秒数);空格暂停、←/→ 切换。
  - **锁定沉浸**:默认锁定,工具栏**不随鼠标弹出**;按 **Esc 解锁**弹出工具栏,**再按 Esc 或点 ✕ 退出**,点 🔒 重新锁定并回全屏。底部计数是「第几屏/共几屏 · 共几张」——竖屏拼图后屏数少于照片数是正常的(照片一张不少)。
  ...
  - 深链 `?ss=1` 打开即自动播放(可加 `&fill=1` 铺满、`&fade=毫秒` 调淡入淡出时长,如 `&fade=600` 更柔、`&fade=0` 不淡)。
