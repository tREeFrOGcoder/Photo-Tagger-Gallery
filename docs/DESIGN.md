...
对象:JPEG+RAW 照片库(围绕 Sony A 系列工作流设计),按天存储在 `/path/to/photos`:
...
5. **文件夹可搜索/选择**:外接盘、移动硬盘、Mac 本地都可能是照片源。
...
- `2026.01.01` 是**纯 ARW 天**(无 JPG)→ 扫描规则:只把「直接包含 ≥1 张 JPG 的文件夹」当作天文件夹,其余跳过。
- exFAT 上存在 **`._*` AppleDouble 垃圾文件**(如 `._2026.01.03`)→ 一切扫描必须过滤 `._` 与隐藏文件。
...
- 已补(2026-07-23):tagger 点选过滤器(✕不看/✓只看)、纯 RAW 库直读(内嵌预览+放大全解)、export_xmp LrC 桥(见 LIGHTROOM.md)。
